...
CONFIG = {
    # DeepSeek API配置
    "api_key": "sk-6bfa57029fbc44b9ac4eb38dd61bc695",  # 在这里填入你的DeepSeek API密钥
    "model": "deepseek-chat",
    "temperature": 0.1,  # 较低的温度以获得更确定性的回答
    
    # 数据配置
    "input_dir": "wash_dir",  # 输入目录：使用标题分块清洗后的数据
    "output_dir": "knowledge_graph_data",  # 输出目录
    "batch_size": 8,  # 每批处理的文本块数量
...
class RelationExtractor:
    ...
    async def process_batch(self, client, chunks):
        """并行处理一批文本块"""
        tasks = [self.extract_relations_from_text(client, chunk) for chunk in chunks]
    ...
    async def process_directory_async(self):
        ...
        # 使用tqdm创建文件处理进度条
        tasks = [self.process_file(file_path) for file_path in json_files]
        all_results = []
