...
class Neo4jImporter:
        ...
        try:
            ...
            if kg_data is None:
                if kg_file_path is None:
                    # 尝试在默认目录中查找
                    kg_file_path = f"{self.neo4j_config['kg_dir']}/output/optimized_*.json"
                    files = list(Path(self.neo4j_config['kg_dir']).glob('output/optimized_*.json'))
                    if files:
                        kg_file_path = str(files[0])
                    else:
                        # 没有找到优化后的文件，查找原始文件
                        files = list(Path(self.neo4j_config['kg_dir']).glob('*_relations.json'))
                        if files:
                            kg_file_path = str(files[0])
                        else:
            ...
            # 执行导入
            return self._import_to_neo4j(kg_data)
    ...
    def _import_to_neo4j(self, kg_data):
        """将知识图谱数据导入到Neo4j数据库"""
        if not self.neo4j_driver:
            return False
        ...
        try:
            ...
            with self.neo4j_driver.session(database=self.neo4j_config["database"]) as session:
                ...
                print(f"已导入 {len(relations)} 个关系到Neo4j数据库")
