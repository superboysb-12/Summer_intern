...
if __name__ == "__main__":
    ...
    kg_file = "../knowledge_graph_data/output/optimized_cp09-样章示例-嵌入式Python开发_relations.json"
    output_dir = "../rag_data/embedded_python"
    
    generator.process_kg_file(kg_file, output_dir)
    
    # 测试KGRAGService
    service = KGRAGService()
    service.load_rag_data(output_dir)
    
    # 测试检索
    query = "树莓派的GPIO接口有什么用途？"
    results = service.retrieve(query, top_k=5)
    
    print("\n检索结果:")
    for result in results:
        print(f"[{result['rank']}] {result['triple']} (相似度: {result['similarity']:.4f})")
        
    # 测试实体信息获取
    entity = "树莓派"
    entity_info = service.get_entity_info(entity)
    
    print(f"\n实体 '{entity}' 的信息:")
    print(f"- 出度: {entity_info['out_degree']}")
    print(f"- 入度: {entity_info['in_degree']}")
    print(f"- 关系类型: {', '.join(entity_info['relation_types'])}")
    
    print("\n出边关系:")
    for rel in entity_info['outgoing_relations']:
        print(f"- {entity} {rel['relation']} {rel['target']}")
    
    print("\n入边关系:")
    for rel in entity_info['incoming_relations']:
        print(f"- {rel['source']} {rel['relation']} {entity}")
